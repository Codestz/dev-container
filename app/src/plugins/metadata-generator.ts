...
function shouldTagElement(elementName: string): boolean {
  // TODO: Add logic to determine if the element should be tagged
  return true;
}
...
export function componentTagger(): Plugin {
  ...
  return {
    ...
    enforce: "pre",
    configResolved() {
      console.log("Component tagger initialized");
    },
        ...
        walk(ast as any, {
          enter(node: any) {
            ...
            if (jsxNode.type === "JSXOpeningElement") {
              ...
              // Get start position from the opening element
              const startLine = jsxNode.loc?.start?.line ?? 0;
              const startCol = jsxNode.loc?.start?.column ?? 0;

              // Get end position from the current element (which includes the closing tag)
              const endLine = currentElement?.loc?.end?.line ?? startLine;
              const endCol = currentElement?.loc?.end?.column ?? startCol;

              const dataComponentId = `${relativePath}:${startLine}:${startCol}`;
              const fileName = path2.basename(id);

              if (shouldTagElement(elementName)) {
                const legacyIds = ` data-component-path="${relativePath}" data-component-line="${startLine}" data-component-file="${fileName}" data-component-name="${elementName}"`;
                const extendedPositionInfo = ` data-line-start="${startLine}" data-line-end="${endLine}" data-col-start="${startCol}" data-col-end="${endCol}"`;

                magicString.appendLeft(
                  jsxNode.name?.end ?? 0,
                  ` data-archie-id="${dataComponentId}" data-archie-name="${elementName}"${extendedPositionInfo}${legacyIds}`
                );
    ...
    closeBundle() {
      // Report tagging statistics at the end of the build
      console.log("\nComponent Tagger Results:");
      console.log(
